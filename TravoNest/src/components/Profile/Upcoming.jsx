...
const Upcoming = () => {
  ...
  return (
    <div style={{width:'100vw',height:'70vh', display:'flex',flexDirection:'column',alignItems:'center',marginTop:'50px',marginBottom:'30px'}}>
          <h3 style={{textAlign:'center',fontWeight:'800',color:'lightgray'}}>Packed up your bag for your upcoming Adventures</h3>
            ...
            (da.length > 0)?
                  da.sort((a, b) => Number(a.startDate) - Number(b.startDate)).map( item => {
                    ...
                    if(pro.booked.includes(item._id) && (tourDate > currentDate)){
                      return (
                        <div key={item._id} className={item._id} style={{width:'60vw',height:'200px',paddingTop:'10px',paddingBottom:'10px',borderRadius:'5px',display:'flex',alignItems:'center',justifyContent:'space-between',backgroundColor:'#F6F5F2',boxShadow:'inset 0px 0px 5px  gray'}} >  
